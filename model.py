...
def WeeklyRandomWalk(name,n,initial,flt=np.array([1.,1.,1.,1.,1.,1.,1.],dtype=np.float64),sigma=.05,offset=0):
...
with pm.Model() as model:
    ...
    #             name="sigma_random_walk_a", sigma=pr_d["pr_sigma_random_walk"]
    #         )
    #         lambda_t_random_walk = pm.distributions.timeseries.GaussianRandomWalk(
    #             name="lambda_t_random_walk_a",
    #             mu=0,
    ...
    #     else:
    #         lambda_base = lambda_list[0] * tt.ones(mi.length)
    #
...
#    f_unknown = pm.Lognormal(name="f_unknown",mu=tt.log(1.1),sigma=.1)
    ...
    onset_of_illness_t = est_deaths(infected_t,median=(4.5),sigma=.3,factor=1.,l=12)[:mi.length]
...
#    sigma_delayed_obs = pm.Lognormal(name="sigma_delayed_obs",mu=tt.log(10),sigma=.8)
    pm.StudentT(
            ...
            nu=4,
            mu=per_day_curves_s[4:],
            sigma=tt.abs_(per_day_curves_s[4:] + tt.alloc(1,per_day_curves_s.shape[1])) ** 0.5 * sigma_onset_obs,  # +1 and tt.abs to avoid nans
    ...
    if False:
        ...
        pm.StudentT(
                ...
                nu=4,
                mu=onset_of_illness_t[4:-12],
                sigma=tt.abs_(onset_of_illness_t[4:-12] + 1) ** 0.5 * sigma_reported_obs,  # +1 and tt.abs to avoid nans
                observed=reported
            )
